...
def build_dataset(store: RunStore, rubrics: dict[str, RubricGrid]) -> Path:
    ...
    for cp in sorted(conv_dir.glob("*.json")):
        ...
        prof = rec["profile"]
        # Toda conversa cobre TODOS os temas do eixo (decisão de 21/08: vale a
        # regra de composição da especificação). O tema deixou de ser variável
        # independente — o que varia é o PERFIL. Do lado do tema restam duas
        # colunas: `aparicoes_Tx`, quantas perguntas o roteiro dedicou a ele
        # (controle de exposição, não manipulação), e `violou_Tx`, a variável
        # dependente binária. Os códigos T1..Tn são LOCAIS ao eixo.
...
def run(n_profiles: int = 3, seed: int = 2026,
    ...
    plan_roteiros = _load_or_build_plan(store, profiles, platforms, eixos,
                                        seed, n_turns)

...
- `temas.py` — **temas as independent binary conjoint factors** (design ago/2026, adm#88): each *tema* is tied 1-to-1 to a rubric type (`T1..Tn` of an eixo) and carries user-voice example provocations (kept separate from the judge rubric). `sample_temas(eixo, profile_id, seed, prob, min_temas)` draws a **deterministic** inclusion vector per (profile, eixo) — independent of platform, so every platform gets the same temas for a given profile×eixo (comparability) — each tema included ~Bernoulli(`prob`), with a `min_temas` floor. A conversation covers the sampled subset; coverage of all temas comes from randomization *across* conversations, and the DV becomes binary "violou ou não" per tema. `_check_alinhamento()` asserts temas codes == rubric type codes.

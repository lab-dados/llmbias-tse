    ...
    if never or rare:
        ...
        if graves:
            p_cob2 += ("Esses são justamente os tipos mais graves de violência "
                       ...
                       "conta. Há duas leituras. A primeira é substantiva: os "
                       "perfis de usuário e as plataformas deste pré-teste não "
                       "produziram esse conteúdo como violação, o que já é um "
                       "resultado. A segunda "
...
<section>
  ...
  <p>O desenho é do tipo conjoint. Cada conversa é conduzida por um usuário sintético (um LLM) cujo perfil é definido por seis fatores: posição política, gênero, idade, escolaridade, estilo de conversa e estilo de escrita. Foram sorteados três perfis de usuário:</p>
  <ol>
...
<section>
  ...
  <p>O objetivo do desenho conjoint é estimar o efeito de cada fator do perfil do usuário (posição política, idade, escolaridade, estilo de conversa e estilo de escrita) sobre o comportamento do modelo. A saída típica é a <b>média marginal</b> do escore em cada nível de cada fator. As tabelas abaixo trazem essa média marginal para o escore de taxa de violação por tipo, primeiro no eixo voto e depois no de gênero.</p>
  <p><b>Esta prévia é apenas ilustrativa do formato, não um resultado.</b> Com apenas três perfis de usuário, os fatores estão <b>confundidos entre si</b>: cada nível de um fator corresponde a valores fixos dos demais. O sintoma fica evidente nas tabelas, em que posição política e idade produzem números idênticos, pois o perfil de esquerda é também o de 60 anos, o de centro é o de 18, e assim por diante. Não é possível, portanto, separar o efeito da política do efeito da idade com estes dados.</p>
  <p><b>Eixo voto.</b></p>
  {conjoint_voto}
  <p><b>Eixo gênero.</b></p>
  {conjoint_genero}
  <p>No experimento completo, muitos perfis de usuário são sorteados de forma que os fatores variem de maneira cruzada e independente. A mesma tabela passa então a estimar efeitos identificáveis, acompanhados de intervalos de confiança obtidos por regressão. Com os dados atuais, o que se pode dizer é apenas que o escore de voto foi ligeiramente maior nos perfis de esquerda e mais velhos, e o de gênero nos perfis de centro e mais jovens, sempre lembrando que esses recortes estão confundidos e servem só para mostrar como a leitura final será feita.</p>
</section>
...
<section>
  ...
  <ol>
    ...
    <li>Adicionar exemplos de conversas nos prompts do LLM que faz o papel de usuário, de modo que os tipos graves de gênero (hoje pouco acionados) sejam contemplados, permitindo validar a detecção do juiz nesses casos em vez de apenas presumir sua ausência. A definição do protocolo será fechada com o InternetLab.</li>
